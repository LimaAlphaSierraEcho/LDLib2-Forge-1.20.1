...
import com.lowdragmc.lowdraglib2.utils.function.Consumers;
...
public class GraphView extends UIElement {
    ...
    protected void buildUITree(GraphModel graphModel) {
        ...
        for (var placeholder : graphModel.getPlaceholders()) {
            ...
            if (placeholder instanceof WirePlaceHolder wirePlaceHolder) {
                createWireUI(wirePlaceHolder);
                continue;
            }
            if (placeholder instanceof NodePlaceholder nodePlaceholder) {
...
//        m_MarkersParent.Clear();
    ...
    protected void onDragEnd(UIEvent event) {
        if (event.dragHandler.draggingObject instanceof DragMove dragMove) {
            ...
            var localOffset = getContentViewContainer().getLocalMouseNormal(offset.x, offset.y);
            dispatchCommand(new GraphCommands.MoveElementsCommand(new ArrayList<>(dragMove.movables), localOffset));
        }
    }
    ...
    protected void onGraphViewDragSourceUpdate(UIEvent event) {
        if (event.dragHandler.getDraggingObject() instanceof DragRegionSelection dragRegionSelectionRecord) {
            var selectionRect = dragRegionSelectionRecord.selectionRect();
