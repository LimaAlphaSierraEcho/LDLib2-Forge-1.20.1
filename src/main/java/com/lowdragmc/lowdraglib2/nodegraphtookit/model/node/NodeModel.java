...
public abstract class NodeModel extends InputOutputPortsNodeModel implements INodeWithOptions {
    ...
    public PortModel getReusablePort(@Nullable OrderedPorts ports,
        ...
        if (graphModel != null) {
            ...
            if (model instanceof PortModel found) {
                portModel = found;
            }
        } else if (ports != null) {
            portModel = ports.get(PortModel.computeUniqueName(portId, parentPort == null ? null : parentPort.getUniqueName()));
        }
        if (portModel != null) {
    ...
    public PortModel getOrCreateTypeConflictPlaceholder(PortDirection direction, String basePortId) {
        ...
        if (infos.portsById.get(id) instanceof PortModel) return infos.portsById.get(id);
    ...
    private void reviveTypeConflictPlaceholders(PortInfos infos) {
        for (var candidate : new ArrayList<>(infos.portsById.values())) {
            if (!(candidate instanceof PortModel)
                    || !candidate.getPortType().equals(PortType.MISSING_PORT)) continue;
            ...
            if (!(base instanceof PortModel)
                    || base.getPortType().equals(PortType.MISSING_PORT)) continue;
